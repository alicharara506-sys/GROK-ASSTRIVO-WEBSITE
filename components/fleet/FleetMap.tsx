...
import { CORE_MEMBER, SPECIALISTS, type FleetMember } from "@/lib/fleet";
...
export function FleetMap() {
  const { hover, hoveredId } = useFleetUi();
    ...
    >
      <div className="mx-auto max-w-6xl">
        ...
        <p className="mt-4 max-w-2xl text-base leading-relaxed text-muted sm:text-lg">
          One core. Ten specialists. One spine. Open any specialist to see
          overview, services, and work — a visual kit, not a chat.
        </p>
        ...
        <div className="mt-14 flex flex-col gap-3 sm:hidden">
          <AgentCard
            member={CORE_MEMBER}
            featured
            active={hoveredId === CORE_MEMBER.slug}
            onHover={(on) => hover(on ? CORE_MEMBER.slug : null)}
          />
        ...
        <div className="relative mx-auto mt-14 hidden aspect-square w-full max-w-[44rem] sm:block">
          ...
          >
            {SPECIALISTS.map((member, index) => {
              const { angle } = polar(index, SPECIALISTS.length, 40);
          ...
          {SPECIALISTS.map((member, index) => {
            const { left, top } = polar(index, SPECIALISTS.length, 40);
            return (
              <div
                key={member.slug}
  ...
  return (
    <h3 className="m-0 font-normal">
      <Link
        ...
        className={`panel-glass block w-full rounded-2xl px-2.5 py-3 text-left transition duration-200 sm:px-3 sm:py-3.5 ${
          featured ? "shadow-[0_0_40px_rgba(255,43,214,0.28)]" : ""
        } ${active ? "ring-1" : ""}`}
        style={{
          borderColor: active ? member.theme.secondary : undefined,
          boxShadow: active
            ? `0 0 24px ${member.theme.secondary}40`
            : undefined,
        }}
      >
        <span className="flex items-center gap-2">
          <span
            ...
            style={{ borderColor: `${member.theme.secondary}40` }}
          >
